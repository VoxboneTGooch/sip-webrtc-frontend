...
var PERMITTED_FIELDS = [
  ...
  'confirmation', 'email', 'reference', 'ringtone', 'browserNotifications'
];
...
router.post('/edit', utils.isLoggedIn, function (req, res) {
  ...
  Account.findOne({_id: req.user._id}, function (err, theAccount) {
    ...
    theAccount.browserNotifications = formData.browserNotifications;
...
router.post('/reset/:token', function (req, res) {
  async.waterfall([
    ...
    function (account, done) {
      ...
      email.subject = 'Sip2Webr.tc - Your password has been changed';
      email.html = ' ';
